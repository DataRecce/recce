 ...
 * @description Comprehensive tests for useMultiNodesAction hook
 *
 * Tests verify:
...
describe("useMultiNodesAction", () => {
  ...
  describe("runRowCount", () => {
    ...
    it("skips non-model nodes with 'Not a model' reason", async () => {
      ...
      const { result } = renderHook(() =>
        useMultiNodesAction([sourceNode, modelNode], defaultCallbacks),
      );
    ...
    it("calls submitRun with correct params for model nodes", async () => {
      ...
      const { result } = renderHook(() =>
        useMultiNodesAction([node1, node2], defaultCallbacks),
      );
  ...
  describe("runRowCountDiff", () => {
    ...
    it("skips non-model nodes with 'Not a model' reason", async () => {
      ...
      const { result } = renderHook(() =>
        useMultiNodesAction([seedNode], defaultCallbacks),
      );
  ...
  describe("runValueDiff", () => {
    ...
    it("skips nodes without primary_key", async () => {
      ...
      const { result } = renderHook(() =>
        useMultiNodesAction([nodeWithoutPK], defaultCallbacks),
      );
    ...
    it("calls submitRun with value_diff type and correct params", async () => {
      ...
      const { result } = renderHook(() =>
        useMultiNodesAction([node], defaultCallbacks),
      );
  ...
  describe("reset", () => {
    it("resets core actionState properties to initial values", async () => {
      ...
      // Note: actions is reset to initValue.actions which is a shared reference
      // The hook uses Object.assign which copies the reference, not creates a new object
    });

    it("resets status allowing a new run to start fresh", async () => {
      const nodes = [createMockNode()];
      mockSubmitRun.mockResolvedValue({ run_id: "test-run-id" });
      mockWaitRun.mockResolvedValue(createMockRun({ result: { total: 100 } }));
      ...
      // Verify the action was tracked for the node we ran
      const nodeId = nodes[0].id;
      expect(result.current.actionState.actions[nodeId]).toBeDefined();
      expect(result.current.actionState.actions[nodeId].status).toBe("success");

      act(() => {
        result.current.reset();
      });

      // After reset, status is pending and we can run again
      expect(result.current.actionState.status).toBe("pending");
    });
  ...
  describe("integration", () => {
    it("handles mixed node types correctly", async () => {
      const modelNode = createMockNode({
        id: "model.test.model",
        resourceType: "model",
      });
      ...
      // Model should have success status
      expect(result.current.actionState.actions[modelNode.id].status).toBe(
        "success",
      );

      // Source and seed should be skipped
      expect(result.current.actionState.actions[sourceNode.id].status).toBe(
        "skipped",
      );
      expect(result.current.actionState.actions[seedNode.id].status).toBe(
        "skipped",
      );
    });

    it("tracks currentRun during execution", async () => {
      const nodes = [createMockNode()];
      mockSubmitRun.mockResolvedValue({ run_id: "tracked-run-id" });

      let capturedCurrentRun: unknown = null;
      mockWaitRun.mockImplementation(() => {
        capturedCurrentRun = { run_id: "tracked-run-id" };
        return Promise.resolve(
          createMockRun({ run_id: "tracked-run-id", result: { total: 100 } }),
        );
      });
      ...
      expect(capturedCurrentRun).toBeDefined();
    });

    it("supports running multiple different actions sequentially", async () => {
      const nodes = [createMockNode()];
      mockSubmitRun.mockResolvedValue({ run_id: "test-run-id" });
      mockWaitRun.mockResolvedValue(createMockRun({ result: {} }));
      ...
      // Reset and run different action
      act(() => {
        result.current.reset();
      });
      ...
      expect(mockSubmitRun).toHaveBeenLastCalledWith(
        "row_count_diff",
        expect.any(Object),
        expect.any(Object),
        mockApiClient,
