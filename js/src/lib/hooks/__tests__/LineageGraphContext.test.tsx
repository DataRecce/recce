 ...
 * @description Tests for LineageGraphContext provider and hooks
 *
 * Phase 2A: Context Unification - Establishing behavioral contracts
 * These tests verify the behavior of LineageGraphContextProvider and useLineageGraphContext
 * to ensure nothing breaks during migration.
 *
 * The OSS LineageGraphContext has:
 * - Internal data fetching via React Query (getServerInfo, aggregateRuns)
 * - WebSocket connection handling via useLineageWatcher
 * - UI modals for server disconnection
 * - Lineage graph state management
 */
...
>;
const mockUseIdleTimeout = useIdleTimeout as jest.MockedFunction<
  typeof useIdleTimeout
>;
...
describe("LineageGraphContext", () => {
  ...
  describe("default context (outside provider)", () => {
    it("useLineageGraphContext returns default values outside provider", () => {
      ...
      expect(screen.getByTestId("is-demo-site")).toHaveTextContent("false");
      expect(screen.getByTestId("action-available")).toHaveTextContent("true");
    });
  });
