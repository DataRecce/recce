...
vi.mock("@datarecce/ui/components/run", () => ({
  ...
  // Stage B (DRC-3390) gated ``findByRunType`` calls behind ``isRegisteredRunType``
  // inside RecceActionAdapter. The mock replaces the whole module, so we need
  // to stub the guard too — return true for the run types the tests exercise
  // so the adapter doesn't bail out before calling submitRun.
