...
describe("toSchemaDataGrid profile modes", () => {
  const schemaDiff = {
    amount: {
      name: "amount",
      baseIndex: 1,
  ...
  } as unknown as Parameters<typeof toSchemaDataGrid>[0];
  ...
  >([
    [
      "amount",
      {
        base: { not_null_proportion: 0.98, avg: 42.1 },
        current: { not_null_proportion: 0.96, avg: 42.1 },
      },
    ],
  ]);
