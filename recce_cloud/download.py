...
def download_from_existing_session(console, token: str, session_id: str, target_path: str, force: bool = False):
    ...
    console.print(f'Downloading manifest to "{manifest_path}"')
    try:
        ...
        console.print("[red]Error:[/red] Failed to download manifest.json")
        console.print(f"Reason: {e}")
        sys.exit(4)

    # Download catalog.json
    console.print(f'Downloading catalog to "{catalog_path}"')
    try:
        ...
        console.print("[red]Error:[/red] Failed to download catalog.json")
        console.print(f"Reason: {e}")
        sys.exit(4)
...
def download_with_platform_apis(console, token: str, ci_info, target_path: str, force: bool = False):
    ...
    This workflow uses get-recce-session-download-urls to find and download artifacts.
    """
    ...
    try:
        ...
        if not session_id or not manifest_download_url or not catalog_download_url:
            console.print("[red]Error:[/red] Incomplete response from get-recce-session-download-urls API")
    ...
    console.print(f'Downloading manifest to "{manifest_path}"')
    try:
        response = requests.get(manifest_download_url)
        if response.status_code != 200:
            raise Exception(f"Download failed with status {response.status_code}: {response.text}")
        with open(manifest_path, "wb") as f:
            f.write(response.content)
    ...
    console.print(f'Downloading catalog to "{catalog_path}"')
    try:
        response = requests.get(catalog_download_url)
        if response.status_code != 200:
            raise Exception(f"Download failed with status {response.status_code}: {response.text}")
        with open(catalog_path, "wb") as f:
            f.write(response.content)
