...
def test_query_diff_in_client(dbt_test_helper):
    ...
    params = dict(sql_template=f'select * from {{{{ ref("customers") }}}}')  # noqa: F541
...
def test_query_diff_in_client(dbt_test_helper):
    ...
    params = dict(
        base_sql_template=f'select * from {{{{ ref("customers") }}}} where customer_id <= 2',  # noqa: F541
        sql_template=f'select * from {{{{ ref("customers") }}}}',  # noqa: F541
    )
    task = QueryDiffTask(params)
    run_result = task.execute()
...
def test_query_diff_in_warehouse(dbt_test_helper):
    ...
    dbt_test_helper.create_model("customers", csv_data_base, csv_data_curr)
    params = dict(sql_template=f'select * from {{{{ ref("customers") }}}}', primary_keys=["customer_id"])  # noqa: F541
    task = QueryDiffTask(params)
    run_result = task.execute()
    assert len(run_result.diff.data) == 2

    params = dict(
        base_sql_template=f'select * from {{{{ ref("customers") }}}} where customer_id == 1',  # noqa: F541
        sql_template=f'select * from {{{{ ref("customers") }}}}',  # noqa: F541
