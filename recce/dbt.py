...
class DBTContext:
    ...
    @classmethod
    def load_from_pkl(cls, artifact_path: str = 'recce.pkl'):
        try:
            context = cls(
                artifact_path=artifact_path,
            )
            context.load_artifacts_from_file()
            context.artifacts_files = [os.path.abspath(artifact_path)]
    ...
    def load(cls, **kwargs):
        ...
        recce_pkl_path = kwargs.get('pkl_path', './recce.pkl')
        ...
        if not parse_result.success:
            if os.path.isfile(recce_pkl_path):
                return cls.load_from_pkl(recce_pkl_path)
            sys.exit(1)
    ...
    def load_artifacts_from_file(self):
        if self.artifact_path is None:
            raise Exception('The artifact path is not set')
    ...
    def _get_lineage_from_artifact(self, base: Optional[bool] = False):
        if self.artifact is None:
            return None
        return self.artifact.get('base') if base is True else self.artifact.get('current')
