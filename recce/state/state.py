...
class RecceStateMetadata(BaseModel):
    # schema version bump to v1 since recce v1.24.2
...
class RecceState(BaseModel):
    ...
    def from_json(json_content: str):
        ...
        if metadata:
            ...
            if metadata.schema_version == "v0":
                # after v1.24.0, DataFrame columns have "key" field to meet react-data-grid requirement
                # the migration result is written back to the state file when export state file
                RecceState._migrate_state_file_runs(state)
            elif metadata.schema_version == "v1":
                pass
            else:
                raise RecceException(f"Unsupported state file version: {metadata.schema_version}, "
                                     f"generated from: {metadata.recce_version}. Please upgrade your Recce.")
        return state
    ...
    @staticmethod
    def _migrate_state_file_runs(state):
        for run in state.runs:
            if run.result is None:
                continue
            if RunType(run.type) in [RunType.QUERY_DIFF, RunType.PROFILE_DIFF]:
                base_columns = run.result.get("base", {}).get("columns", [])
                current_columns = run.result.get("current", {}).get("columns", [])
                for column in base_columns:
                    column["key"] = column["name"]
                for column in current_columns:
                    column["key"] = column["name"]
            elif RunType(run.type) in [RunType.QUERY, RunType.QUERY_BASE, RunType.VALUE_DIFF_DETAIL]:
                columns = run.result.get("columns", [])
                for column in columns:
                    column["key"] = column["name"]
            elif RunType(run.type) == RunType.VALUE_DIFF:
                data_columns = run.result.get("data", {}).get("columns", [])
                for column in data_columns:
                    column["key"] = column["name"]
            elif RunType(run.type) == RunType.PROFILE:
                current_columns = run.result.get("current").get("columns", [])
                for column in current_columns:
                    column["key"] = column["name"]
