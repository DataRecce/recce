...
def extract_rows_from_artifacts(
    ...
    env: Env,
    *,
    cross_env_catalog: Optional[dict] = None,
) -> tuple[list[NodeRow], list[ColumnRow], list[EdgeRow], list[NodeTestRow]]:
    """Produce flat row lists from a (manifest, catalog) pair.

    Column source priority per node, mirroring OSS recce semantics
    (catalog-only — never schema.yml fallback):

      1. ``catalog`` (this env). Generated by ``dbt docs generate`` against
         the warehouse, so its column list IS the materialized table.
      2. ``cross_env_catalog`` (the other env's catalog, if provided). Used
         when this env's catalog has no entry for the node — typical when
         the user ran ``dbt docs generate --select`` and only changed
         models got a fresh catalog. The other env's catalog is the closest
         proxy for what OSS's live ``get_columns()`` query would have
         returned for this env. Casing is correct (warehouse identifier
         rules are env-independent for the same warehouse) and for
         unchanged models the column list is identical.
      3. **Empty.** When neither catalog has the node, this function
         emits zero column rows for it. Matches OSS bulk lineage
         (``DbtAdapter.get_lineage_cached``) which only attaches a
         ``columns`` field when ``catalog.nodes`` has an entry — there's
         no schema.yml fallback in OSS, so we don't have one either.

    ``manifest`` must be a raw dict (e.g. ``json.load(manifest.json)``).
    ``catalog`` and ``cross_env_catalog`` may be ``None``.

    Tests are NOT part of columns — they're emitted separately as
    ``NodeTestRow``. Tests on columns we didn't store are dropped (orphan
    rows would not join to ``columns`` downstream).
    """
    catalog_nodes = ((catalog or {}).get("nodes") or {}) if catalog else {}
    cross_env_catalog_nodes = ((cross_env_catalog or {}).get("nodes") or {}) if cross_env_catalog else {}
    ...
    for node_id, node in _iter_manifest_nodes(manifest):
        ...
        # Cross-env catalog fallback. When this env's catalog has no entry,
        # borrow the other env's catalog. For unchanged models the column
        # list and types are identical between envs; for modified models
        # the casing is still warehouse-truth (identifier folding doesn't
        # vary between envs of the same warehouse) even if a column was
        # added/removed. This is the cloud equivalent of OSS's live
        # ``get_columns()`` against the warehouse — the warehouse table
        # state is shared infrastructure, not env-specific.
        column_source = catalog_cols
        if not column_source and cross_env_catalog_nodes:
            cross_node = cross_env_catalog_nodes.get(node_id)
            column_source = (cross_node or {}).get("columns") or {}

        # Dedup is case-insensitive: Snowflake catalog folds identifiers
        # to UPPERCASE while a sibling catalog from a partial docs build
        # might have a different casing for the same column. Whichever
        # form we see first wins.
        ordered_cols: list[str] = []
        seen_canonical: set[str] = set()
        for col_name, col_info in column_source.items():
